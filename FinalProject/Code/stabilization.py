...
def extract_harris_corners(gray_img, max_corners=150, thresh=0.01):
    ...
    # Connected Components: Groups pixel clusters (blobs) and extracts their center-of-mass centroids
    _, _, _, centroids = cv2.connectedComponentsWithStats(dst_thresh)

    # Centroid Slicing: Discards index 0, which represents the entire black background canvas component
    corners = centroids[1:]

    # Optimization 1: Distribute tracking points evenly into an outer grid layout (4x4 grid)
    ...
    # Optimization 2: Refine integer centroids to fractional sub-pixel coordinates
    if len(corners) > 0:
...
def estimate_pair_transform(prev_gray, curr_gray, lk_params):
    ...
    # Local Tracking: Lucas-Kanade independently maps the local spatial displacement of each individual feature
    ...
    # Global Modeling: Fits a rigid 4-DOF Affine matrix using RANSAC to filter out human outlier motion vectors
    if len(valid_prev) >= 4:
...
def stabilize_video_pipeline(input_path, output_path, window_size=11):
    ...
    for i in range(num_frames - 1):
        ...
        # Optimization 3: Accumulate the transformation chain back to Frame 0 coordinate space
